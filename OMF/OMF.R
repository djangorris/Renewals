...
OMF_2018_rates_clean <- read_excel("OMF/OMF-2018-rates3.xls") %>%
  gather(Var, Val, Plan:Total) %>%
  group_by(Name, Var) %>%
  mutate(n = row_number()) %>%
  unite(VarT, Var, n, sep="") %>%
  spread(VarT, Val, fill=0) %>%
  # MAKE SURE TO MATCH THE NUMBER OF PLANS CHOSEN
...
glimpse(OMF_2018_rates_clean)

OMF_2018_rates_mutated <- mutate(OMF_2018_rates_clean, EE_Portion1 = as.numeric(format(round((Full_EE_Premium1 * 0.35), 2), nsmall = 2)),
                                 EE_Portion2 = as.numeric(format(round((Full_EE_Premium2 * 0.35), 2), nsmall = 2)),
                                 EE_Portion3 = as.numeric(format(round((Full_EE_Premium3 * 0.5), 2), nsmall = 2)),
...
OMF_2018_rates_final <- select(OMF_2018_rates_mutated, Name, Plan1, Full_EE_Premium1, EE_Portion1, Employer_Portion1, Dependents1, Family_Portion1, Total1,
